# HINGE
**Household Implicit Negotiation-free Grace Endeavor**
...
HINGE is a fortnightly rotation that answers one question every day: *which parent is Primary today?*
...
**Grace is the coverage rule.** Life happens. When a parent is sick, traveling, or otherwise unavailable, the other parent absorbs the day. No swap log, no IOU, no rebalancing conversation. This is the grace in HINGE — freely given, not tracked.
...
The default answer to a swap request is *yes, of course.* HINGE is a generous system. Both parents are invested in making it work, which means making it work for each other. A swap is not a withdrawal from a shared account — it is an expression of the same grace that underlies the coverage rule.
...
A categorical is any recurring exception type. HINGE handles categoricals by rule, not by case-by-case negotiation. The rules are defined here, once, and applied automatically.
