# ORBIT
**Our Rotation, Both In Turn**
*Not a chore chart. An operating agreement.*

---

## What This Is

ORBIT is a fortnightly rotation that answers one question every day: *which parent is Primary today?*

**Primary** is not a title — it is a responsibility. The Primary parent owns the day end to end: morning launch, afternoon pickup, after-school obligations, and whatever comes up in between. The other parent is present and available but not on point. They have the day to themselves. This single distinction — clearly assigned, known in advance, never negotiated in the moment — is what makes everything else in ORBIT work.
...
**Blocks are the unit.** A single free day is a gift. Two or more contiguous free days are a different thing entirely — they have momentum. Projects resume, context reloads, work compounds. ORBIT guarantees each parent a meaningful block of free days every week.
...
**Grace is the coverage rule.** Life happens. When a parent is sick, traveling, or otherwise unavailable, the other parent absorbs the day. No swap log, no IOU, no rebalancing conversation. This is the grace in ORBIT — freely given, not tracked.

**Categoricals flow through.** Exceptions do not break the pattern. Every recurring exception type has a handling rule defined in advance. When something comes up, the question is not *what do we do* — it is *which rule applies.*

**The schedule is always knowable.** Any day, months from now, has a named Primary. You can book appointments, plan travel, and schedule personal commitments with confidence.
...
ORBIT comes in two block-length variants. Both share the same active week (Mon–Sat), the same ISO week anchor, the same principles, and the same categorical rules. The only difference is how many days constitute a block. The variant is chosen deliberately per season — neither is the default.
...
A swap is when one parent takes another's Primary day, and the other parent takes one of theirs in return — or simply covers without expectation of return.

The default answer to a swap request is *yes, of course.* ORBIT is a generous system. Both parents are invested in making it work, which means making it work for each other. A swap is not a withdrawal from a shared account — it is an expression of the same grace that underlies the coverage rule.

**How it works in practice:**

- One parent identifies a day they need covered and asks.
- The other parent says yes. The swap happens.
...
A categorical is any recurring exception type. ORBIT handles categoricals by rule, not by case-by-case negotiation. The rules are defined here, once, and applied automatically.

| Categorical | What it means | Rule |
|---|---|---|
